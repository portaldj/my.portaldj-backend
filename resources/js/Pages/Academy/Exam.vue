    ...
    <AuthenticatedLayout>
        ...
        <div class="py-12">
            <div class="mx-auto max-w-4xl sm:px-6 lg:px-8">
                <div class="bg-brand-surface overflow-hidden shadow-sm sm:rounded-lg border border-gray-700 p-8">
                    <form @submit.prevent="submitExam">
                        <div v-for="(question, index) in exam.questions" :key="question.id" class="mb-8">
                            <h3 class="text-lg font-bold text-gray-200 mb-4">{{ index + 1 }}. {{ question.text }}</h3>
                            ...
                            <div class="space-y-2">
                                <div v-for="answer in question.answers" :key="answer.id" class="flex items-center">
                                    <input 
                                        ...
                                        class="w-4 h-4 text-brand-primary bg-gray-700 border-gray-600 focus:ring-brand-primary focus:ring-2"
                                    >
                                    <label :for="`q${question.id}_a${answer.id}`" class="ml-2 text-sm font-medium text-gray-300 cursor-pointer">
                                        {{ answer.text }}
                                    </label>
                                </div>
                            </div>
