...
const activateTrial = () => {
    if (confirm(__('Start your 7-Day Free Trial?'))) {
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div v-if="isPro && activeSubscription" class="bg-green-100 dark:bg-green-900 border border-green-400 text-green-700 dark:text-green-300 px-4 py-3 rounded relative mb-6" role="alert">
                    <strong class="font-bold">{{ __('You are a PRO Member!') }}</strong>
                    <span class="block sm:inline"> {{ __('Your subscription is active until :date.', { date: expiryDate }) }}</span>
                    <p class="mt-2 text-sm">{{ __('To renew or change your plan, please wait until your current subscription expires.') }}</p>
                </div>
                ...
                <div v-else class="text-center mb-10">
                    ...
                    <p class="mt-4 text-xl text-gray-600 dark:text-gray-400">
                        {{ __('Unlock full access to the Academy, Music Pool, and AI Assistant.') }}
                    </p>
                </div>
                    ...
                    <!-- Free Trial Card -->
                     <div class="col-span-1 md:col-span-2 bg-gradient-to-r from-gray-800 to-gray-900 rounded-2xl shadow-xl border border-gray-700 p-6 flex flex-col md:flex-row items-center justify-between">
                        <div class="mb-4 md:mb-0">
                            <h3 class="text-xl font-bold text-white">{{ __('7-Day Free Trial') }}</h3>
                            <p class="text-gray-300">{{ __('Try all PRO features for free. No credit card required.') }}</p>
                            <p v-if="!canStartTrial" class="text-xs text-red-400 mt-1">
                                {{ __('(Available once every 3 months. Next available: :date)', { date: new Date(new Date(user.last_trial_at).setMonth(new Date(user.last_trial_at).getMonth() + 3)).toLocaleDateString() }) }}
                            </p>
                        </div>
                        <button 
                            @click="activateTrial" 
                        ...
                        >
                            {{ canStartTrial ? __('Start Free Trial') : __('Trial Unavailable') }}
                        </button>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden border border-gray-200 dark:border-gray-700 flex flex-col">
                        <div class="p-8 flex-1">
                            ...
                            <p class="mt-4 flex items-baseline text-gray-900 dark:text-white">
                                ...
                                <span class="ml-1 text-xl font-semibold text-gray-500">/{{ __('month') }}</span>
                            </p>
                            <p class="mt-6 text-gray-500 dark:text-gray-400">{{ __('Perfect for getting started.') }}</p>
                            ...
                            <ul role="list" class="mt-6 space-y-4">
                                <li class="flex">
                                    ...
                                    <span class="ml-3 text-gray-500 dark:text-gray-400">{{ __('Full Academy Access') }}</span>
                                </li>
                                <li class="flex">
                                    <svg class="flex-shrink-0 h-6 w-6 text-green-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                    </svg>
                                    <span class="ml-3 text-gray-500 dark:text-gray-400">{{ __('Unlimited Music Pool') }}</span>
                                </li>
                                <li class="flex">
                                    <svg class="flex-shrink-0 h-6 w-6 text-green-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                    </svg>
                                    <span class="ml-3 text-gray-500 dark:text-gray-400">{{ __('AI Assistant') }}</span>
                                </li>
                            </ul>
                        </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden border-2 border-brand-accent flex flex-col relative">
                        ...
                        <div class="p-8 flex-1">
                            ...
                            <p class="mt-4 flex items-baseline text-gray-900 dark:text-white">
                                ...
                                <span class="ml-1 text-xl font-semibold text-gray-500">/{{ __('year') }}</span>
                            </p>
                            <p class="mt-6 text-gray-500 dark:text-gray-400">
                                {{ __('Save :savings% compared to monthly.', { savings: yearly.savings_pct }) }}
                            </p>
